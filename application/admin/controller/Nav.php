...
class Nav extends Admin {
    ...
    public function index() {
        ...
        builder('list')
                ...
                ->addAction('删除', 'delete', '<i class="layui-icon layui-icon-delete"></i>', 'ajax-get confirm layui-btn-danger',' onclick="del(this)" ')
                ->build();
        return view();
    }
...
//                $this->success('添加成功', url('nav/index'));
                return ['code'=>0,'msg'=>'添加成功！','url'=> url('nav/index')];
            } else {
//                $this->error('添加失败');
            ...
            builder('form','aaa')
                    ->addItem('cid', 'select', '分类<span  style="color:red;" >*</span>', model('nav_cat')->column('id,title'), 'lay-verify="required"')
                    ->addItem('title', 'input', '标题<span  style="color:red;" >*</span>', '', 'lay-verify="required"')
    ...
    public function doedit(){
        ...
        if (request()->isPost()) {
            ...
            $post['create_time']=$t;
            $post['update_time']=$t;
            $res=db('nav')->where('id', $id)->update($post);
            if ($res) {
...
//                    ->build($one);
    ...
    public function category() {
        ...
        builder('list')
                ...
                ->addAction('删除', 'category_delete', '<i class="layui-icon layui-icon-delete"></i>', 'ajax-get confirm layui-btn-danger',' onclick="del(this)" ')
                ->build();
        return view();
    }
...
//                $this->success('添加成功', url('nav/category'));
                return ['code'=>0,'msg'=>'添加成功','url'=>url('nav/category')];
            } else {
                $this->error('添加失败');
            }
        } else {
            builder('form')
                    ->addItem('title', 'input', '标题<span  style="color:red;" >*</span>', '', 'lay-verify="required"')
                    ->addItem('alias', 'input', '别名<span  style="color:red;" >*</span>', '', 'lay-verify="required"', '', '用于前端调用')
                    ->build();
            return view();
        }
    }
    ...
    public function category_edit($id) {
        ...
        if (request()->isPost()) {
            ...
            if (db('nav_cat')->where('id', $id)->update($post) !== FALSE) {
                return ['code'=>0,'msg'=>'修改成功','url'=>url('nav/category')];
            } else {
                $this->error('修改失败');
            }
        } else {
    ...
    public function category_delete($id) {
        ...
        if (db('nav_cat')->where('id', $id)->delete() !== FALSE) {
            $this->success('删除成功', url('category'));
        } else {
            $this->error('删除失败');
        }
    }
