...
function sendEmail($recv_email, $nickname ="", $subject = "新意轩摄影", $msgHTML = "XiYiXuan") {
    ...
    //发送邮箱验证码
    $toemail = $recv_email; //定义收件人的邮箱

    $mail = new PHPMailer();

    $mail->isSMTP(); // 使用SMTP服务
    $mail->CharSet = "utf8"; // 编码格式为utf8，不设置编码的话，中文会出现乱码
    $mail->Host = $config['smtp_host']; // 发送方的SMTP服务器地址
    $mail->SMTPAuth = true; // 是否使用身份验证
    $mail->Username = $config['smtp_user']; // 发送方的163邮箱用户名，就是你申请163的SMTP服务使用的163邮箱</span><span style="color:#333333;">
    $mail->Password = $config['smtp_pass']; // 发送方的邮箱密码，注意用163邮箱这里填写的是“客户端授权密码”而不是邮箱的登录密码！</span><span style="color:#333333;">
    $mail->SMTPSecure = "ssl";  // 使用ssl协议方式</span><span style="color:#333333;">
    $mail->Port = $config['smtp_port']; // 163邮箱的ssl协议方式端口号是465/994

    $mail->setFrom($config['smtp_email'], $config['smtp_name']); // 设置发件人信息，如邮件格式说明中的发件人，这里会显示为Mailer(xxxx@163.com），Mailer是当做名字显示
    $mail->addAddress($toemail, $nickname); // 设置收件人信息，如邮件格式说明中的收件人，这里会显示为Liang(yyyy@163.com)
    $mail->addReplyTo($config['smtp_email'], $config['smtp_name']); // 设置回复人信息，指的是收件人收到邮件后，如果要回复，回复邮件将发送到的邮箱地址
 ...
 * @param $siteconfig
 */
function sendRegEmail($nickname, $email, $activate) {

    $siteconfig = config('base');

    if (!isset($siteconfig['title']) || !isset($siteconfig['domain'])) {
        return '配置信息不全';
    }

    $contentstr = "亲爱的用户：<span style='color:#F30;'>" . $nickname . "</span> 您好！ <br />";
    $contentstr .= "感谢您注册【" . $siteconfig['title'] . "】！ <br />";
    $contentstr .= "您现在可以激活您的账户，激活成功后，您可以使用【" . $siteconfig['title'] . "】提供的各种服务。  <br />";
    $contentstr .= "<a href='" . $siteconfig["domain"] . "/activate?code=" . $activate . "' target='_blank'>点此激活账户 </a> <br />";
    $contentstr .= "如果上述文字点击无效，请把下面网页地址复制到浏览器地址栏中打开 <br />";
    $contentstr .= "" . $siteconfig["domain"] . "/activate?code=" . $activate . " <br />";
    $contentstr .= "此为系统邮件，请勿回复";

    return sendEmail($email, $nickname, $siteconfig['title'] . "【账号激活】邮件", $contentstr);
}
...
function admin_is_login() {
    ...
    $admin = authcode($admin, 'DECODE', 'XinYiXuan');
...
function member_is_login() {
    ...
    $member = authcode($member, 'DECODE', 'XinYiXuan');
 ...
 * @return type
 */
function my_md5($value, $salt = '1234') {
