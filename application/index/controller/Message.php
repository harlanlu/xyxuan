...
class Message extends Base {
    ...
    public function lists() {
        ...
        foreach ($lists as $key => $value) {
            ...
            $lists2[$key]['send_id'] = $value['send_id'];
            $lists2[$key]['nickname'] = $value['nickname'];
            $lists2[$key]['content'] = '<a data-id="'.$value['send_id'].'" class="uid" href="'.url('/u/'.$value['send_id']).'" target="_blank">'.$value['nickname'].'</a>' .'：'. $value['message'];
    ...
    public function send() {
        ...
        if (empty($msg)) {
            return ['code' => 1, 'msg' => '发送成功', 'data' => $msg];
        } else {
