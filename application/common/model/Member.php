...
class Member extends Base {
    ...
    public function points($post, $id) {
        $data['points'] = $post['points'];
        Db::startTrans();
        try {

            db('member')->where('id', $id)->data('points', $data['points'])->update();
            // 提交事务
            Db::commit();
        } catch (\Exception $e) {
            // 回滚事务
    ...
    public function password_reset($post, $id) {
        ...
        $count = db('member')->where('id', $id)->where('password', my_md5($post['old_password']))->count();
        if (empty($count)) {
            return '旧密码不正确';
        }
        return db('member')->where('id', $id)->setField('password', my_md5($post['password']));
    }
    ...
    public function login($post) {
        ...
        $member = db('member')
                ->where('email', $email)
                ->field('id,email,nickname,avatar,sex,password')
                ->find();

        if ($member) {
//            $this->session_update($member['id']);
            $this->session_update($member['id']);
            if ($member['password'] == my_md5($password)) {
                $this->session_update($member['id']);
            } else {
                return '输入的密码有误';
            }
...
//            return '登录失败';
    ...
    public function session_update($member_id) {
        ...
        // 保存cookie
        cookie('member', authcode(json_encode($member), 'ENCODE', '1234'));
        //
    }
